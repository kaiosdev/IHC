...
| **C7.** Necessidade de um **feed de atualizações de editais** das pró-reitorias. | **S7.** O controle de presença de orientandos é essencial (Must-Have) e pode ser um "check-in" semanal, mais simples que um controle diário. | **D7.** Com que frequência exata o cliente busca por novos editais (diariamente é literal?)? |
| **C8.** A integração (leitura) com o banco de dados do **e-Campus** é desejável. | **D8.** Como o cliente é notificado sobre a aprovação de um projeto atualmente (Email? Portal?)? |
| **C9.** Projetos frequentemente se relacionam entre si (ex: "PIBICs que conversam com PIBIT"). | | **D9.** Uma funcionalidade de "contatos úteis" (pró-reitorias) pré-cadastrados no app seria útil? |
